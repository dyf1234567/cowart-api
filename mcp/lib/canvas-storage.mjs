...
// ---------------------------------------------------------------------------
// Cowart image provider preferences and machine-local provider configuration.
//
// Model preferences live inside the project canvas directory so skills can
// read which image provider the canvas UI selected. Provider credentials live
// in the user's machine-local Cowart config directory and never inside the
// project.
// ---------------------------------------------------------------------------
...
export function resolveCowartConfigDir() {
  return resolve(
...
export function resolveProviderConfigFile() {
  return join(resolveCowartConfigDir(), "provider-config.json");
}

export function resolveModelPreferencesFile(args = {}) {
  return join(resolveCanvasDir(args), "cowart-model-preferences.json");
}

export function defaultCowartModelPreferences() {
  return {
    version: 1,
...
export async function readCowartModelPreferences(args = {}) {
  const preferencesFile = resolveModelPreferencesFile(args);
  try {
    const preferences = await readJsonFile(preferencesFile);
    if (!isModelPreferences(preferences)) {
      return { preferences: defaultCowartModelPreferences(), preferencesFile };
    }
    return { preferences, preferencesFile };
  } catch (error) {
    if (error?.code === "ENOENT") {
      return { preferences: defaultCowartModelPreferences(), preferencesFile };
    }
...
export async function writeCowartModelPreferences(args = {}, preferences) {
  if (!isModelPreferences(preferences)) {
    throw new Error("Expected Cowart model preferences.");
  }
  const preferencesFile = resolveModelPreferencesFile(args);
  const payload = {
    ...preferences,
  ...
  return { ok: true, path: preferencesFile, preferences: payload };
}
...
function normalizeProviderSection(section, defaults) {
  const merged = { ...defaults };
  if (section && typeof section === "object") {
    for (const [key, value] of Object.entries(defaults)) {
      if (typeof value === "number") {
        if (typeof section[key] === "number" && Number.isFinite(section[key])) merged[key] = section[key];
...
async function readLegacyDashscopeConfig() {
  const legacyFile = join(resolveCowartConfigDir(), "dashscope-config.json");
  try {
    const legacy = await readJsonFile(legacyFile);
    if (legacy && typeof legacy === "object" && typeof legacy.apiKey === "string") {
      return {
...
export async function readCowartProviderConfig() {
  ...
  const config = {
    dashscope: normalizeProviderSection(raw.dashscope, defaults.dashscope),
    custom: normalizeProviderSection(raw.custom, defaults.custom),
    comfyui: normalizeProviderSection(raw.comfyui, defaults.comfyui),
  };

  if (!config.dashscope.apiKey) {
    const legacy = await readLegacyDashscopeConfig();
    if (legacy) config.dashscope = legacy;
  }
...
function maskApiKey(apiKey) {
  ...
  if (!trimmed) return { configured: false, apiKeyPreview: "" };
  return {
    configured: true,
    apiKeyPreview:
      trimmed.length > 10 ? `${trimmed.slice(0, 6)}...${trimmed.slice(-4)}` : `${trimmed.slice(0, 2)}***`,
  };
}

export function publicProviderConfig(config) {
  const safe = config && typeof config === "object" ? config : {};
  const dashscope = safe.dashscope ?? {};
  const custom = safe.custom ?? {};
  const comfyui = safe.comfyui ?? {};
  return {
    dashscope: {
      ...maskApiKey(dashscope.apiKey),
      baseUrl: typeof dashscope.baseUrl === "string" ? dashscope.baseUrl : "",
      model:
        typeof dashscope.model === "string" && dashscope.model.trim()
          ? dashscope.model
          : DEFAULT_DASHSCOPE_MODEL,
    },
    custom: {
    ...
    comfyui: {
      configured: Boolean(
        (typeof comfyui.serverUrl === "string" && comfyui.serverUrl.trim()) ||
          (typeof comfyui.checkpoint === "string" && comfyui.checkpoint.trim()) ||
          (typeof comfyui.workflow === "string" && comfyui.workflow.trim()),
      ),
      serverUrl:
        typeof comfyui.serverUrl === "string" && comfyui.serverUrl.trim()
          ? comfyui.serverUrl
          : DEFAULT_COMFYUI_SERVER_URL,
      checkpoint: typeof comfyui.checkpoint === "string" ? comfyui.checkpoint : "",
      workflow: typeof comfyui.workflow === "string" ? comfyui.workflow : "",
      promptNodePath: typeof comfyui.promptNodePath === "string" ? comfyui.promptNodePath : "",
      negativeNodePath: typeof comfyui.negativeNodePath === "string" ? comfyui.negativeNodePath : "",
      imageNodePath: typeof comfyui.imageNodePath === "string" ? comfyui.imageNodePath : "",
      denoise:
        typeof comfyui.denoise === "number" && Number.isFinite(comfyui.denoise)
          ? comfyui.denoise
          : 0.75,
    },
...
export async function writeCowartProviderConfig(patch = {}) {
  const { config: existing } = await readCowartProviderConfig();
  const next = {
    dashscope: { ...existing.dashscope },
    custom: { ...existing.custom },
    comfyui: { ...existing.comfyui },
  };

  if (patch.dashscope && typeof patch.dashscope === "object") {
    const section = patch.dashscope;
    const apiKey = typeof section.apiKey === "string" ? section.apiKey.trim() : "";
    if (apiKey) next.dashscope.apiKey = apiKey;
    if (typeof section.baseUrl === "string" && section.baseUrl.trim()) {
      next.dashscope.baseUrl = section.baseUrl.trim().replace(/\/+$/, "");
    }
    if (typeof section.model === "string" && section.model.trim()) {
      next.dashscope.model = section.model.trim();
    }
  }

  if (patch.custom && typeof patch.custom === "object") {
    const section = patch.custom;
    const apiKey = typeof section.apiKey === "string" ? section.apiKey.trim() : "";
    if (apiKey) next.custom.apiKey = apiKey;
    if (typeof section.baseUrl === "string" && section.baseUrl.trim()) {
  ...
  if (patch.comfyui && typeof patch.comfyui === "object") {
    const section = patch.comfyui;
    if (typeof section.serverUrl === "string" && section.serverUrl.trim()) {
      next.comfyui.serverUrl = section.serverUrl.trim().replace(/\/+$/, "");
    }
    for (const key of ["checkpoint", "workflow", "promptNodePath", "negativeNodePath", "imageNodePath"]) {
      if (typeof section[key] === "string") next.comfyui[key] = section[key];
    }
    if (typeof section.denoise === "number" && Number.isFinite(section.denoise)) {
      next.comfyui.denoise = Math.min(1, Math.max(0, section.denoise));
    }
  }
