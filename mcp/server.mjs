...
function registerCowartStateTools(mcpServer) {
  ...
  mcpServer.registerTool(
    ...
    async (input = {}) => {
      const result = await readCowartModelPreferences(input);
      return {
        content: [
          {
            type: "text",
            text: `Cowart image provider is "${result.preferences.imageProvider}" (${result.preferencesFile}).`,
          },
        ],
        structuredContent: result,
      };
  ...
  mcpServer.registerTool(
    TOOL_SAVE_MODEL_PREFERENCES,
    {
      title: "Save Cowart Model Preferences",
      description:
        "Persist the Cowart image provider preference selected in the canvas UI to canvas/cowart-model-preferences.json.",
      inputSchema: {
        ...projectArgsSchema,
        preferences: z.any(),
      },
    ...
    async (input = {}) => {
      try {
        const result = await writeCowartModelPreferences(input, input.preferences);
        return {
          content: [
            {
              type: "text",
              text: `Saved Cowart model preferences to ${result.path}.`,
            },
          ],
          structuredContent: result,
        };
  ...
  mcpServer.registerTool(
    TOOL_SAVE_PROVIDER_CONFIG,
    {
      title: "Save Cowart Provider Config",
      description:
        "Merge and persist Cowart image provider configuration (dashscope, custom, comfyui sections) into the machine-local Cowart config directory. Empty apiKey values keep the previously stored key.",
      inputSchema: {
        ...projectArgsSchema,
        config: z.any(),
      },
    ...
    async (input = {}) => {
      try {
        const result = await writeCowartProviderConfig(input.config ?? {});
        return {
          content: [
            {
              type: "text",
