  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/model-preferences', async (req, res) => {
        try {
          const storageArgs = { projectDir, canvasDir }

          if (req.method === 'GET') {
            const result = await readCowartModelPreferences(storageArgs)
            sendJson(res, 200, {
              preferences: result.preferences,
              path: result.preferencesFile
            })
            return
          }
          ...
          if (req.method === 'PUT') {
            ...
            const result = await writeCowartModelPreferences(storageArgs, preferences)
      ...
      server.middlewares.use('/api/provider-config', async (req, res) => {
        try {
          if (req.method === 'GET') {
          ...
          if (req.method === 'PUT') {
            ...
            const result = await writeCowartProviderConfig(patch)
            sendJson(res, 200, { ok: true, config: result.config, path: result.path })
            return
          }
