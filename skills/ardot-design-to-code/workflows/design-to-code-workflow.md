> Codex port: read [runtime compatibility](../../ardot-design-core/references/codex-compatibility.md) before using this reference. Live tool schemas and that contract govern host behavior and API examples.
...
After code generation, run appropriate build and browser verification using the current browser skill.

---

## Phase 0: Resolve the output contract

Use the user's requested stack and the existing project's conventions. Resolve output directory, viewport, and responsive versus fixed layout from context. For a standalone artifact without a specified stack, state a plain HTML/CSS/JS default and proceed. Ask only for consequential missing choices; no repeated confirmation is required for already authorized work.
...
```
export_nodes(fileUrl: exactFileUrl, nodeIds: [...imageNodeIds], format: "png", scale: 1)
Download returned URLs into <project>/assets/images.
```
...
Call `export_nodes` with fileUrl, nodeIds and format: "svg"; download returned URLs into <project>/assets/svg.
