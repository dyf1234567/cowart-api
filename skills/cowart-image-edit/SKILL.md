...
5. Generate a new bitmap.
   ...
   node scripts/generate-custom-api-image.mjs --prompt "<edit instructions>" --reference <clean-source-image-path>
   node scripts/generate-comfyui-image.mjs --prompt "<edit instructions>" --reference <clean-source-image-path>
   ```

   Each script prints JSON with `outputPath`; use that exact local image path for insertion. If the selected provider script fails or is not configured, tell the user what is missing instead of silently switching providers. Do not overwrite the source image file. Save the new bitmap with a timestamped filename, for example:
