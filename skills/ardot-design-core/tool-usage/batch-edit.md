...
Images are fills on frame/rectangle nodes. Remote MCP G creates a gray placeholder with a short label (at most 20 characters) in the user's language. It does not generate real images. Use an available image workflow and register_assets/upload_images with live schemas for real imagery.

```javascript
avatar=I("parentId", {type: "frame", name: "Avatar", width: 120, height: 120})
G(avatar, "placeholder", "头像")
```
